...
import { searchMods, installMod, generateModConfig, applyModsToServer, getModInstallInstructions, getInstalledMods, updateModOrder, uninstallMod, toggleMod, getModDescription, type ModConfigPreview } from '../utils/tauri';
        ...
        <div className="space-y-8 animate-in fade-in duration-500 relative pb-20">
            ...
            {selectedModDetail && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-200" onClick={() => setSelectedModDetail(null)}>
                    <div className="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-5xl h-[85vh] overflow-hidden flex flex-col shadow-2xl shadow-sky-900/20" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col md:flex-row gap-6 justify-between items-center">
                {activeTab === 'available' ? (
                    <div className="relative w-full md:w-96">
                        ...
                        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search CurseForge mods..." className="w-full pl-12 pr-4 py-3 bg-slate-800/50 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all" />
                    </div>
                ) : (
                        ...
                        availableMods.map((mod) => (
                            <div key={mod.id} onClick={() => setSelectedModDetail(mod)} className={cn("glass-panel rounded-2xl overflow-hidden group hover:border-sky-500/50 transition-all flex flex-col cursor-pointer relative", selectedModIds.has(mod.id) ? "border-sky-500/80 ring-2 ring-sky-500/20 bg-sky-900/10" : "")}>
                                <div className="relative h-48 overflow-hidden">
                                    ...
                                    <div onClick={(e) => { e.stopPropagation(); handleToggleSelect(mod.id); }} className="absolute top-3 left-3 z-30 p-2 rounded-lg bg-black/40 hover:bg-black/60 backdrop-blur-sm transition-colors cursor-pointer group/check">
                                ...
                                <div className="p-6 flex-1 flex flex-col pointer-events-none">
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-700/50 pointer-events-auto">
                                        <button onClick={(e) => { e.stopPropagation(); handleInstallMod(mod); }} className="flex items-center space-x-2 px-4 py-2 bg-sky-600/20 hover:bg-sky-500 hover:text-white text-sky-400 border border-sky-500/30 rounded-lg transition-all text-sm font-medium z-30"><Download className="w-4 h-4" /><span>Install</span></button>
                                    </div>
                                </div>
                            </div>
                        ))
